...
- (void)didTapOnPOI:(NSString *)arg_placeID name:(NSString *)arg_name location:(FGMPlatformLatLng *)arg_location completion:(void (^)(FlutterError *_Nullable))completion {
  NSString *channelName = [NSString stringWithFormat:@"%@%@", @"dev.flutter.pigeon.google_maps_flutter_ios.MapsCallbackApi.didTapOnPOI", _messageChannelSuffix];
  FlutterBasicMessageChannel *channel =
    [FlutterBasicMessageChannel
      messageChannelWithName:channelName
      binaryMessenger:self.binaryMessenger
      codec:FGMGetMessagesCodec()];
  [channel sendMessage:@[arg_placeID ?: [NSNull null], arg_name ?: [NSNull null], arg_location ?: [NSNull null]] reply:^(NSArray<id> *reply) {
    if (reply != nil) {
      if (reply.count > 1) {
        completion([FlutterError errorWithCode:reply[0] message:reply[1] details:reply[2]]);
      } else {
